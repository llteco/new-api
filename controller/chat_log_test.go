...
func TestAdminGetChatLogs_ForbiddenForNonAdmin(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	r := gin.New()
	r.GET("/chat_logs", func(c *gin.Context) { c.Set("role", common.RoleCommonUser); c.Next() }, AdminGetChatLogs)
	r.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/chat_logs", nil))
