...
// ponytail: UnixNano alone collides on Windows' coarse clock and the stats
// bucket is global, so keys need a process-unique counter suffix.
...
func TestObserveChannelAffinityUsageCacheByRelayFormat_ClaudeMode(t *testing.T) {
	ruleName := fmt.Sprintf("rule_%d", time.Now().UnixNano()) + fmt.Sprintf("_%d", channelAffinityTestSeq.Add(1))
	usingGroup := "default"
	keyFP := fmt.Sprintf("fp_%d", time.Now().UnixNano()) + fmt.Sprintf("_%d", channelAffinityTestSeq.Add(1))
